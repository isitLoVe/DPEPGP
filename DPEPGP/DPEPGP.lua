...
function DPEPGP_Slash(msg)
	...
	if (playerRank == 0 or playerRank == 1) then -- Guild Leader or Vice Leader
		...
		DPEPGP_Loot:Enable();
		DPEPGP_Decay:Enable();
	elseif playerRank == 2 then -- Officer
		DPEPGP_Raid:Enable();
		DPEPGP_Loot:Enable();
		DPEPGP_Decay:Disable();
	elseif (playerRank == 3 or playerRank == 4) then -- Class Officer or Officer Alt
		DPEPGP_Raid:Disable();
		DPEPGP_Loot:Enable();
		DPEPGP_Decay:Disable();
	else -- Member(5) or Casual(6) or Alt(7) or Aspirant(8)	
		DPEPGP_Raid:Disable();
		DPEPGP_Loot:Disable();
		DPEPGP_Decay:Disable();
	end		
